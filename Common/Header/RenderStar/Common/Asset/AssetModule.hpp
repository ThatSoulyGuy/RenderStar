...
{
    class AssetModule : public Module::AbstractModule
    {
    public:
        AssetModule();
        explicit AssetModule(const std::filesystem::path& basePath);
    ...
    AssetHandle<T> AssetModule::Load(const AssetLocation& location)
    {
        ...
        auto cachedAsset = cache.Get<T>(location);
        if (cachedAsset)
        ...
        auto loaderIt = loaders.find(std::type_index(typeid(T)));
        if (loaderIt == loaders.end())
            return AssetHandle<T>();

        auto loader = std::static_pointer_cast<IAssetLoader<T>>(loaderIt->second);
